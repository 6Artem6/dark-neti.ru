...
class ActiveUsersRecord extends Widget
{
	...
	public function run()
	{
		...
		$item_list[] = Html::tag('span', $text, [
			...
			'data-toggle' => 'tooltip',
			'title' =>  Yii::t('app', 'Количество ответов пользователя за неделю')
		]);

		$text = HtmlHelper::getCountText($this->comment_count);
		$item_list[] = Html::tag('span', $text, [
			'class' => ['bi', 'bi-chat', 'bi_icon', 'text-secondary'],
			'data-toggle' => 'tooltip',
			'title' =>  Yii::t('app', 'Количество комментариев пользователя за неделю')
		]);
