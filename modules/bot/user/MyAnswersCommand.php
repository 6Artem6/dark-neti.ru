...
class MyAnswersCommand extends BaseUserCommand
{

	protected $name = 'myanswers';
	protected $description = 'Список моих ответов';
	protected $usage = '/myanswers';

	public const COMMAND_TEXT = 'Мои ответы';
	public const COMMAND_NAME = 'myanswers';
	...
	protected function sendList(int $chat_id, int $page = 0, int $question_id = 0)
	{
		...
		$chat = UserChat::findByChat($chat_id);
		$count = $model->getAllUserCount($chat->user_id);
		$list = $model->getAllUserList($chat->user_id, $limit, $offset, $question_id);
