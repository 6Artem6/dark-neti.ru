...
class SupportCommand extends BaseAdminCommand
{

	protected $name = 'support';
	protected $description = 'Управление обращениями в поддержку';
	protected $usage = '/support';

	public const COMMAND_TEXT = 'Обращения';
	public const COMMAND_NAME = 'support';
	...
	protected function sendMain(int $chat_id = 0)
	{
		...
		$keyboard->addRow([
			'text' => Yii::t('app', 'Показать все обращения'),
		...
		if ($today_count) {
			$keyboard->addRow([
				'text' => Yii::t('app', 'Показать обращения за сегодня'),
				'callback_data' => json_encode([
					'command' => static::COMMAND_NAME,
	...
	protected function sendList(int $chat_id, int $page = 0)
	{
		...
		foreach ($list as $record) {
			...
			$keyboard = new InlineKeyboard([]);
			$keyboard->addRow([
				'text' => Yii::t('app', 'Показать обращение подробнее'),
				'callback_data' => json_encode([
					'command' => static::COMMAND_NAME,
		...
		$text = Yii::t('app', 'Обращения: {begin}-{end} из {count}', [
			'begin' => $begin, 'end' => $end, 'count' => $count
		]);
		$keyboard = new InlineKeyboard([]);
		if ($offset != 0) {
			$keyboard->addRow([
				'text' => Yii::t('app', 'Предыдущие обращения'),
				'callback_data' => json_encode([
					'command' => static::COMMAND_NAME,
					'action' => static::ACTION_ALL,
					'page' => ($page - 1)
		...
		if (($limit + $offset) < $count) {
			$keyboard->addRow([
				'text' => Yii::t('app', 'Следующие обращения'),
				'callback_data' => json_encode([
					'command' => static::COMMAND_NAME,
					'action' => static::ACTION_ALL,
					'page' => ($page + 1)
