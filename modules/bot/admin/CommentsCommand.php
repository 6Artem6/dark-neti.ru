...
class CommentsCommand extends BaseAdminCommand
{

	protected $name = 'comments';
	protected $description = 'Список комментариев';
	protected $usage = '/comments';

	public const COMMAND_TEXT = 'Комментарии';
	...
	protected function sendMain(int $chat_id = 0)
	{
		...
		$message_text[] = [
			'text' => 'Скрыто комментариев',
			'count' => $model->getHiddenCount()
		];
		$message_text[] = [
			'text' => 'Удалено комментариев',
			'count' => $model->getDeletedCount()
		];
		/*$message_text[] = [
			'text' => 'Непроверенных комментариев',
			'count' => $model->getAllCount()
		];
		$message_text[] = [
			'text' => 'Одобрено комментариев',
			'count' => $model->getAllCount()
		];*/
	...
	protected function sendList(int $chat_id, int $page = 0, int $question_id = 0, int $answer_id = 0)
	{
		...
		$offset = $page * $limit;
		$model = new Comment;
		$count = $model->getAllCount($question_id, $answer_id);
		$list = $model->getAllList($limit, $offset, $question_id, $answer_id);
